...
const SET_REPETS: usize = 10;
const GET_REPETS: usize = 10000;
...
fn set_bench(c: &mut Criterion) {
    ...
    c.bench_function("set_kvs", move |b| {
        b.iter(|| set_n_times(&mut kv_store, SET_REPETS))
    });

    c.bench_function("set_sled", move |b| {
        b.iter(|| set_n_times(&mut sled_db, SET_REPETS))
    });
}
...
fn get_bench(c: &mut Criterion) {
    ...
    set_n_times(&mut kv_store, GET_REPETS);
    set_n_times(&mut sled_db, GET_REPETS);

    c.bench_function("get_kvs", move |b| {
        b.iter(|| get_n_times_randomly(&mut kv_store, GET_REPETS))
    });

    c.bench_function("get_sled", move |b| {
        b.iter(|| get_n_times_randomly(&mut sled_db, GET_REPETS))
    });
}
...
fn get_n_times_randomly<E: KvsEngine>(engine: &mut E, n: usize) {
    ...
    for _ in 0..n {
        engine
            .get(format!("key{}", rng.gen_range(0, GET_REPETS)))
            .unwrap();
    }
}
